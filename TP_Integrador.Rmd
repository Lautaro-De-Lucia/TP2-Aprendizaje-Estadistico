...
results <- data.frame(k = 1:100, accuracy = numeric(100))

for (k in 1:100) {
  knn_model <- knn(train = sat_trn_df[,1:4], test = sat_tst_df[,1:4], cl = sat_trn_df[,5], k = k)
  
  accuracy <- mean(knn_model == sat_tst_df[,5]) * 100
  
  results[k, ] <- c(k, accuracy)
}
