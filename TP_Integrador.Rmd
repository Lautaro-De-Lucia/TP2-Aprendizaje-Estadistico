...
CLErr_tree <- mean(sat_tree_pred != sat_tst_df$C)
#utils.OVAROC(sat_tst_df,sat_tree)
```
...
```{r}
models <- c("tree", "rpart", "RF", "KNN", "RLM","LDA","QDA")
CVErrs <- c(CLErr_tree,CLErr_rpart,CLErr_rf,CLErr_knn,CLErr_mlr,CLErr_LDA,CLErr_QDA)
...
```{r,echo=FALSE,warning=FALSE,message=FALSE}
sat_tree_pred <- predict(sat_tree, sat_tst_full_df, type = "class")
utils.conf_matrix(sat_tst_full_df,sat_tree_pred)
CLErr_tree <- mean(sat_tree_pred != sat_tst_full_df$C)
#utils.OVAROC(sat_tst_df,sat_tree)
```
